...
- ざっくりレイアウトの感じがつかめてきた
- あとは背景色とか奥行関係
- めんどいけど遊戯王っぽい色を決めてTheme作成する、手っ取り早くMarginで見た目も解消
- 参考：https://engawapg.net/jetpack-compose/1457/theme/

20230916
- ふわっとしたデザインはできてきたので、トップページと答えのページを作成する
- ざっくりとした考えでは、リザルト画面（問題数と正答数・率）とトップページ（問題数指定とスタートボタン）
- それぞれ一週間ぐらいで行けそうだが、画面遷移がどうにかなるか

20230927
- Navigation Composeを使うらしい
- なんか眠くて理解できないので、後日考える
- 渡す変数はトップ⇒問題で問題数、問題⇒リザルトで正答数ぐらい
- https://qiita.com/shxun6934/items/5301b69a85df98e30983

20230928
- https://rni-dev.hatenablog.com/entry/2022/09/02/111724
- これが一番わかりやすそう
- Activityは複数準備しないっぽい
- 新規でcomposeの関数を作っていくような気がする、ちゃんと読む

20231002
- navigation composeについてはちゃんとわからず
- https://www.youtube.com/watch?v=_5vP-NIRUCM&list=PLaOdaBFokChw6PA-uCyvhWUP6VLYnmHdd
- たどり着いたこの動画で、uri的な部分の文字列を返すメソッドで引数に受け渡す値を指定することでタイポなどを防げる
- 遷移方法をもう少し調べる、navigation composeでもう一度検索

20231003
- 多分それっぽくはなってきた
- おそらく、現画面の実装を別のktに隔離して、それをcomposable内で呼び出すと予想、指定はroute文字列な気がする
- navControllerの変数だけ不明なので調査

20231004
- ここを参考にしたらとりあえずの実装はできた URL: https://qiita.com/shxun6934/items/5301b69a85df98e30983
- 次に検証端末を充電しておいて、作った画面が出れば成功
- あとはrouteをそれぞれ作るんだが、多分分けて作らないとfatコードになってしまうのでちょっと調べる(Fragmentとか？)

20231010
- 表示成功
- TOPに数字入れて、問題数決めるようにする
- 数字のバリデーションだけ頑張る